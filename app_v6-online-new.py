...
def compute_indicator_crisis_similarity(
    ...
    """
    Indicator-level similarity score (0-100) between today's value and the
    selected crisis window distribution for one indicator.
    """
    start_dt = pd.to_datetime(win_start)
    end_dt = pd.to_datetime(win_end)
    w = df.loc[start_dt:end_dt].copy()

    if w.empty or key not in w.columns or key not in current_row.index:
        return {"fit_pct": np.nan, "label": "No data"}

    hist = pd.to_numeric(w[key], errors="coerce").dropna()
    cur = pd.to_numeric(pd.Series([current_row.get(key, np.nan)]), errors="coerce").iloc[0]

    if pd.isna(cur) or len(hist) < min_points:
        return {"fit_pct": np.nan, "label": "Insufficient history"}
    ...
    if scale is None:
        return {"fit_pct": np.nan, "label": "Unstable"}

    dist = abs(float(cur) - med) / scale
    sim = float(np.exp(-dist))
    fit_pct = 100.0 * sim

    # Current percentile within crisis window
    current_pctile = 100.0 * float((hist <= cur).mean())

    if fit_pct >= 80:
        band = "strong resemblance"
    elif fit_pct >= 60:
        band = "moderate resemblance"
    elif fit_pct >= 40:
        band = "partial resemblance"
    else:
        band = "weak resemblance"
...
def get_crisis_investor_playbook(crisis_name, fit_score):
    """
    Returns practical investor guidance based on historical crisis analog.
    fit_score: 0–100
    """
...
def get_top_crisis_contributors(latest_row: pd.Series, core_keys: list[str], labels_map: dict, top_n: int = 3):
    """
    Return top signed contributors by absolute size from the current crisis-score row.
    """
    items = []
    for k in core_keys:
...
def build_dynamic_crisis_playbook(
    ...
    """
    Build investor guidance tied to the top current crisis contributors.
    """
    top = get_top_crisis_contributors(latest_row, core_keys, labels_map, top_n=3)

    # Base crisis-style guidance from your existing helper
    ...
    lines = []
    lines.append("Driver-based reading of the current setup:")

    if supportive:
        lines.append(
            "- Main crisis-matching drivers now: " + ", ".join(fmt_driver(x) for x in supportive[:3]) + "."
        )
    if negative:
        lines.append(
            "- Main offsets / differences vs the crisis template: " + ", ".join(fmt_driver(x) for x in negative[:3]) + "."
        )

    # Make suggestions depend on sign and fit strength
    if pd.notna(fit_score) and fit_score >= 70:
        lines.append(
            "- Because the overall similarity is high, treat the top supportive drivers as the main regime clues."
        )
    elif pd.notna(fit_score) and fit_score >= 45:
        lines.append(
            "- Similarity is only moderate, so use the top drivers as a guide, but avoid assuming the full crisis path will repeat."
        )
    else:
        lines.append(
            "- Similarity is weak, so keep the historical analogy as a reference only, not as a dominant positioning signal."
        )

    # Gold-specific action framing
    if supportive and not negative:
        lines.append(
            "- Positioning idea: tilt toward the assets that historically benefited in this kind of regime, but scale in gradually."
        )
    elif supportive and negative:
        lines.append(
            "- Positioning idea: use a balanced stance. The crisis analogy is partly present, but some important drivers are not aligned."
        )
    elif negative and not supportive:
        lines.append(
            "- Positioning idea: stay defensive. The current setup is not strongly confirming the historical regime."
        )
...
def build_now_vs_crisis_vertical_figure(
    ...
    Vertical comparison:
    - top = current window
    - bottom = historical crisis window
    with crisis percentile zones shown on both.
    """
    ...
    # ---------- TOP: current ----------
    ax1.plot(cur.index, cur.values, linewidth=1.4, label=current_label)
    ax1.scatter(cur.index[-1], cur.values[-1], s=25, zorder=3)

    ax1.set_ylim(y_min - pad, y_max + pad)
    add_zones(ax1)

    if pd.notna(median):
        ax1.axhline(median, linestyle="--", linewidth=1.0, label="Median during Selected Crisis")

    ax1.set_title("Current window", fontsize=11)
    ...
    # ---------- BOTTOM: crisis ----------
    ax2.plot(cri.index, cri.values, linewidth=1.4, linestyle="--", label=crisis_label)
    ax2.scatter(cri.index[-1], cri.values[-1], s=25, zorder=3)

    ax2.set_ylim(y_min - pad, y_max + pad)
    add_zones(ax2)

    if pd.notna(median):
        ax2.axhline(median, linestyle="--", linewidth=1.0, label="Median during Selected Crisis")

    ax2.set_title("Historical crisis window", fontsize=11)
...
def robust_zscore(series: pd.Series, window: int = 60, min_periods: int = 24) -> pd.Series:
    """
    Rolling z-score with a minimum history requirement.
    Safer for macro series with missing history.
    """
...
def compute_fair_gold_monthly(
    ...
    etf_proxy: pd.Series,     # NEW
    cb_proxy: pd.Series,  # NEW
    vix_series: pd.Series,
    *,
    z_window: int = 36,
    fit_window_months: int = 84,   # 7 years, faster than 10y
    min_fit_obs: int = 24,
) -> pd.DataFrame:
    """
    Variant B: rolling fitted Fair Gold model.

    For each month t:
      1) compute z-scored macro drivers
      2) fit OLS on the trailing fit_window_months sample:
           log(Gold) ~ 1 + Z(real_yield) + Z(usd) + Z(inflation) + Z(stress)
      3) use the fitted coefficients at t to estimate Fair Gold_t

    This allows the macro sensitivities to evolve across regimes.
    """
    tmp = pd.concat(
        [
    ...
    # Rolling z-scores on drivers
    # Use real yield level (structural gold driver)
    tmp["Z_REAL_YIELD"] = robust_zscore(
        tmp["REAL_YIELD_INPUT"],
        window=z_window
    )
    tmp["Z_USD"] = robust_zscore(tmp["USD_INPUT"], window=z_window)
    # 🔧 De-correlated inflation (inflation surprise)
    ...
    # --- NEW: Gold momentum (12M return) ---
    tmp["GOLD_RET_12M"] = tmp["GOLD_USD"].pct_change(12)

    tmp["Z_GOLD_MOM"] = robust_zscore(
        tmp["GOLD_RET_12M"],
        window=z_window
    ).clip(-3, 3)

    # =========================
    # ORTHOGONALIZE MOMENTUM (FIXED)
    # =========================
    ...
    X_orth_cols = [c for c in X_orth_cols if c in tmp.columns]
    ...
    if len(common_idx) > 30:
        X_mat = X_orth.loc[common_idx].values
        ...
        print("Momentum orthogonalized (correct). Residual std:", np.std(mom_resid))

    tmp["VIX"] = pd.to_numeric(vix_series, errors="coerce").reindex(tmp.index)

    # GEO level: medium-term stress regime
    tmp["GEO_LEVEL"] = tmp["VIX"].rolling(3, min_periods=1).mean()

    # GEO shock: sudden escalation
    tmp["GEO_SHOCK"] = tmp["VIX"] - tmp["VIX"].rolling(6, min_periods=3).mean()

    # Blend level + shock so geopolitics is not only raw VIX
    tmp["GEO_INPUT"] = 0.6 * tmp["GEO_LEVEL"] + 0.4 * tmp["GEO_SHOCK"]

    print("VIX exists:", "VIX" in tmp.columns)
    print("GEO_INPUT non-null:", tmp["GEO_INPUT"].notna().sum())
    print(tmp["GEO_INPUT"].tail())

    tmp["Z_GEO"] = robust_zscore(tmp["GEO_INPUT"], window=z_window)

    # scale CB impact (proxy is weak vs reality)
    tmp["Z_CB"] = tmp["Z_CB"] * 1.3

    # CB persistence (captures sustained accumulation)
    ...
    # CB reacts stronger in stress regimes
    tmp["Z_CB_STRESS"] = tmp["Z_CB"] * np.maximum(tmp["Z_STRESS"], 0)

    # Geopolitics amplifies stress regimes
    ...
    reg_cols = core_reg_cols + extra_reg_cols

    # Output containers
    tmp["FAIR_GOLD_Z"] = np.nan
    tmp["FAIR_GOLD_LOG"] = np.nan
    tmp["FAIR_GOLD"] = np.nan

    # Per-driver contribution diagnostics
    tmp["CONTRIB_REAL_YIELD"] = np.nan
    tmp["CONTRIB_USD"] = np.nan
    tmp["CONTRIB_INFL"] = np.nan
    tmp["CONTRIB_STRESS"] = np.nan
    tmp["CONTRIB_LIQ"] = np.nan
    tmp["CONTRIB_ETF"] = np.nan
    tmp["CONTRIB_CB"] = np.nan
    tmp["CONTRIB_GEO"] = np.nan
    tmp["BETA_CB_TREND"] = np.nan

    # Optional: store rolling betas for inspection/debug
    tmp["BETA_INTERCEPT"] = np.nan
    tmp["BETA_RY"] = np.nan
    tmp["BETA_USD"] = np.nan
    tmp["BETA_INFL"] = np.nan
    tmp["BETA_STRESS"] = np.nan
    tmp["BETA_LIQ"] = np.nan
    tmp["BETA_ETF"] = np.nan
    tmp["BETA_CB"] = np.nan
    tmp["BETA_GOLD_MOM"] = np.nan

    # Latest z-value diagnostics for ETF / CB
    tmp["ETF_INPUT_LATEST"] = np.nan
    tmp["CB_INPUT_LATEST"] = np.nan

    tmp["B0_PREV"] = np.nan

    valid = tmp[["GOLD_USD"] + reg_cols].copy()

    for i in range(len(tmp)):
        end_idx = tmp.index[i]

        # trailing rolling window ending at current month
        start_pos = max(0, i - fit_window_months + 1)

        # Apply to current month only
        cur = tmp.loc[end_idx, reg_cols]
        valid_cols = [c for c in core_reg_cols if pd.notna(cur[c])]

        # Add ETF / CB only when available
        extras_available = [c for c in extra_reg_cols if c in valid.columns]

        # 🔧 limit extras to avoid instability and Prevent overfitting (VERY IMPORTANT)
        # Without this: CB, CB_TREND, CB_STRESS are highly correlated ->
        # - Regression becomes unstable
        # - Betas collapse

        # Always include GEO if available
        extras_selected = []

        # Always include GEO
        if "Z_GEO" in extras_available:
            extras_selected.append("Z_GEO")

        # ALWAYS include momentum (critical)
        if "Z_GOLD_MOM" in extras_available:
            extras_selected.append("Z_GOLD_MOM")

        # Fill remaining slots
        for c in extras_available:
            if c not in extras_selected:
                extras_selected.append(c)
            if len(extras_selected) >= 5:
                break
        valid_cols += extras_selected

        # Interaction stability filter
        if "Z_LIQ_STRESS" in valid_cols:
            if not (abs(cur.get("Z_LIQ", 0)) > 0.5 and abs(cur.get("Z_STRESS", 0)) > 0.75):
                valid_cols = [c for c in valid_cols if c != "Z_LIQ_STRESS"]

        print("Z_GEO in valid_cols:", "Z_GEO" in valid_cols)
        print("FINAL valid_cols:", valid_cols)
        print("Z_GOLD_MOM in valid_cols:", "Z_GOLD_MOM" in valid_cols)
        print("FINAL valid_cols:", valid_cols)

        # Regression start

        if len(valid_cols) >= len(core_reg_cols):
            win_full = valid.iloc[start_pos:i + 1].copy()

            if "Z_GEO" not in valid_cols and "Z_GEO" in win_full.columns:
                valid_cols.append("Z_GEO")

            if "Z_GOLD_MOM" not in valid_cols and "Z_GOLD_MOM" in win_full.columns:
                valid_cols.append("Z_GOLD_MOM")

            # Require core only
            win_core = win_full[["GOLD_USD"] + core_reg_cols].dropna()

            if win_core.shape[0] < min_fit_obs:
                continue

            # Add extras only where available
            use_cols = core_reg_cols.copy()

            for c in extra_reg_cols:
                if pd.notna(cur[c]):
                    use_cols.append(c)

            print("Before dropna:", win_full.columns.tolist())

            # the next is where dropna starts

            # FULL regression column set (no valid_cols anymore)
            regression_cols = [
                "Z_REAL_YIELD",
                "Z_USD",
                "Z_INFL",
            ...
            base_cols = ["GOLD_USD"] + regression_cols_available
            base_cols = [c for c in base_cols if c in win_full.columns]
            base_cols = list(dict.fromkeys(base_cols))  # remove duplicates safely

            win2 = win_full[base_cols].copy()

            # Drop rows only if core drivers are missing
            win2 = win2.dropna(subset=["GOLD_USD"] + regression_cols_available)

            # Use same columns for regression
            X_cols = regression_cols_available.copy()

            # AFTER win2 is fully constructed
            print("win2 columns:", win2.columns.tolist())
            print("Z_GOLD_MOM in win2:", "Z_GOLD_MOM" in win2.columns)

            if "Z_GOLD_MOM" in win2.columns:
                print("Z_GOLD_MOM non-null:", win2["Z_GOLD_MOM"].notna().sum())

            print("Z_GEO in win2:", "Z_GEO" in win2.columns)
            print("win2 size:", win2.shape)
            print("win2 rows:", len(win2))
            print("NaN counts:")
            print(win2.isna().sum())
            print("Z_GOLD_MOM in win2:", "Z_GOLD_MOM" in win2.columns)
            ...
            # FORCE full regression column set (no silent dropping)


            # Keep only columns that exist (but DO NOT rely on valid_cols anymore)
            X_cols = [c for c in X_cols if c in win2.columns]

            X = np.column_stack(
                [np.ones(len(win2))] +
                [win2[c].fillna(0.0).astype(float).values for c in X_cols]
            )

            # exponential time weighting (recent data matters more)
            n = len(y)
            weights = np.exp(np.linspace(-2.0, 0.0, n))  # older -> smaller weight
            ...
            # Smooth intercept (prevents full absorption of new signals)
            # Raw intercept from regression
            b0_raw = beta[0]
            ...
            # Safe previous value
            if prev_idx is not None and "B0_PREV" in tmp.columns:
                prev_b0 = tmp.loc[prev_idx, "B0_PREV"]
                if pd.notna(prev_b0):
                    b0 = 0.7 * prev_b0 + 0.3 * b0_raw
                else:
                    b0 = b0_raw
            else:
                b0 = b0_raw
            ...
            print("Z_GOLD_MOM in betas:", "Z_GOLD_MOM" in betas)

            if "Z_GOLD_MOM" in betas:
                print("Z_GOLD_MOM beta:", betas["Z_GOLD_MOM"])

            # Regime-aware stress activation block
            if "Z_STRESS" in betas:
                z_stress = cur.get("Z_STRESS", 0.0)

                # Base: no negative effect
                beta_stress = max(betas["Z_STRESS"], 0.0)

                # Activation: only strong when real stress event
                if abs(z_stress) < 0.75:
                    beta_stress *= 0.2  # mostly inactive
                elif abs(z_stress) < 1.5:
                    beta_stress *= 0.6  # moderate activation
                else:
                    beta_stress *= 1.2  # crisis amplification
            ...
            if "Z_GOLD_MOM" in betas:
                ...
                # hard cap (prevents dominance)
                beta_mom = np.clip(beta_mom, -0.7, 0.7)

                betas["Z_GOLD_MOM"] = beta_mom

            # Build fair_z from contributions ONLY (single source of truth)

            contrib_map = {
                c: betas.get(c, 0.0) * cur.get(c, 0.0)
                for c in valid_cols
            }

            # Base fair_z from regression
            fair_z_core = sum(betas[c] * cur[c] for c in valid_cols if c != "Z_GOLD_MOM")

            fair_z = sum(betas[c] * cur[c] for c in valid_cols)

            fair_log = b0 + fair_z

            # 🔴 CRITICAL: restore FAIR_GOLD assignment
            tmp.loc[end_idx, "FAIR_GOLD_Z"] = fair_z
            tmp.loc[end_idx, "FAIR_GOLD_LOG"] = fair_log
            tmp.loc[end_idx, "FAIR_GOLD"] = np.exp(fair_log)

            # Debug (keep these)
            tmp.loc[end_idx, "DEBUG_FAIR_Z"] = fair_z
            tmp.loc[end_idx, "DEBUG_FAIR_LOG"] = fair_log
            tmp.loc[end_idx, "DEBUG_EXP_MULT"] = np.exp(fair_z)

            # Per-driver contributions to FAIR_GOLD_Z
            tmp.loc[end_idx, "CONTRIB_REAL_YIELD"] = betas.get("Z_REAL_YIELD", 0.0) * cur.get("Z_REAL_YIELD", np.nan)
            tmp.loc[end_idx, "CONTRIB_USD"] = betas.get("Z_USD", 0.0) * cur.get("Z_USD", np.nan)
            tmp.loc[end_idx, "CONTRIB_INFL"] = betas.get("Z_INFL", 0.0) * cur.get("Z_INFL", np.nan)
            tmp.loc[end_idx, "CONTRIB_STRESS"] = betas.get("Z_STRESS", 0.0) * cur.get("Z_STRESS", np.nan)
            tmp.loc[end_idx, "CONTRIB_LIQ"] = betas.get("Z_LIQ", 0.0) * cur.get("Z_LIQ", np.nan)
            tmp.loc[end_idx, "CONTRIB_ETF"] = betas.get("Z_ETF", 0.0) * cur.get("Z_ETF", np.nan)
            tmp.loc[end_idx, "CONTRIB_CB"] = (
                    betas.get("Z_CB", 0.0) * cur.get("Z_CB", 0.0)
                    + betas.get("Z_CB_TREND", 0.0) * cur.get("Z_CB_TREND", 0.0)
            )
            tmp.loc[end_idx, "CONTRIB_GOLD_MOM"] = betas.get("Z_GOLD_MOM", 0.0) * cur.get("Z_GOLD_MOM", np.nan)

            tmp.loc[end_idx, "ETF_INPUT_LATEST"] = cur.get("Z_ETF", np.nan)
            tmp.loc[end_idx, "CB_INPUT_LATEST"] = cur.get("Z_CB", np.nan)
            tmp.loc[end_idx, "CONTRIB_GEO"] = betas.get("Z_GEO", 0.0) * cur.get("Z_GEO", np.nan)
            tmp.loc[end_idx, "CONTRIB_GEO_STRESS"] = betas.get("Z_GEO_STRESS", 0.0) * cur.get("Z_GEO_STRESS", np.nan)

            tmp.loc[end_idx, "BETA_INTERCEPT"] = b0
            if "Z_REAL_YIELD" in betas:
                tmp.loc[end_idx, "BETA_RY"] = betas["Z_REAL_YIELD"]
            if "Z_USD" in betas:
                tmp.loc[end_idx, "BETA_USD"] = betas["Z_USD"]
            if "Z_INFL" in betas:
                tmp.loc[end_idx, "BETA_INFL"] = betas["Z_INFL"]
            if "Z_STRESS" in betas:
                tmp.loc[end_idx, "BETA_STRESS"] = betas["Z_STRESS"]
            if "Z_LIQ" in betas:
                tmp.loc[end_idx, "BETA_LIQ"] = betas["Z_LIQ"]
            if "Z_ETF" in betas:
                tmp.loc[end_idx, "BETA_ETF"] = betas["Z_ETF"]
            if "Z_CB" in betas:
                tmp.loc[end_idx, "BETA_CB"] = betas["Z_CB"]
            if "Z_GEO" in betas:
                tmp.loc[end_idx, "CONTRIB_GEO"] = betas["Z_GEO"] * cur["Z_GEO"]
            if "Z_CB_TREND" in betas:
                tmp.loc[end_idx, "BETA_CB_TREND"] = betas["Z_CB_TREND"]
            if "Z_GOLD_MOM" in betas:
                tmp.loc[end_idx, "BETA_GOLD_MOM"] = betas["Z_GOLD_MOM"]

    tmp["Z_ETF"] = tmp["Z_ETF"].fillna(0.0)
    tmp["Z_CB"] = tmp["Z_CB"].fillna(0.0)
    tmp["CONTRIB_ETF"] = tmp["CONTRIB_ETF"].fillna(0.0)
    tmp["CONTRIB_CB"] = tmp["CONTRIB_CB"].fillna(0.0)
    ...
    # Print latest rolling coefficients for sanity check
    latest_betas = tmp[
        ["BETA_INTERCEPT", "BETA_RY", "BETA_USD", "BETA_INFL", "BETA_STRESS", "BETA_LIQ"]
    ].dropna()

    if not latest_betas.empty:
        last = latest_betas.iloc[-1]
        print("Rolling Fair Gold latest betas:", {
            "intercept": float(last["BETA_INTERCEPT"]),
            "real_yield": float(last["BETA_RY"]),
            "usd": float(last["BETA_USD"]),
            "inflation": float(last["BETA_INFL"]),
            "stress": float(last["BETA_STRESS"]),
            "liquidity": float(last["BETA_LIQ"]),
        })

    latest_cb = tmp[["Z_CB", "BETA_CB", "CONTRIB_CB"]].dropna(how="all")
    if not latest_cb.empty:
        print("Latest CB diagnostics:", latest_cb.iloc[-1].to_dict())

    print("FAIR_GOLD non-null:", int(tmp["FAIR_GOLD"].notna().sum()))
    print("FAIR_GOLD_GAP_PCT non-null:", int(tmp["FAIR_GOLD_GAP_PCT"].notna().sum()))
    print("CONTRIB_REAL_YIELD non-null:", int(tmp["CONTRIB_REAL_YIELD"].notna().sum()))
    print("B0_PREV exists:", "B0_PREV" in tmp.columns)
    print("First values:", tmp["B0_PREV"].head())
...
def build_features(monthly_method: str = "avg") -> pd.DataFrame:
    ...
    # Liquidity proxy: Fed balance sheet (12M % change)
    walcl = safe_fred("WALCL")
    walcl_m = to_monthly(walcl, monthly_method)
    ...
    # ----- ETF demand proxy (prefer GLD shares outstanding; fallback to price-volume flow) -----
    gld_ser, gld_source, gld_err = load_yf_series_fast("GLD", start="2004-01-01")

    if gld_ser is not None and not gld_ser.empty:
        etf_proxy = None

        # 1) Try shares outstanding first
        try:
            gld_tkr = yf.Ticker("GLD")
            shares = gld_tkr.get_shares_full(start="2004-01-01")
            ...
            if shares is not None and not shares.empty:
                ...
                shares_m = shares.resample("ME").last()

                # Reject stale or too-short shares data
                if shares_m.dropna().shape[0] >= 24 and shares_m.index.max() >= (
                        df.index.max() - pd.DateOffset(months=6)):
                    etf_proxy = 100.0 * (shares_m / shares_m.shift(3) - 1.0)
                    etf_proxy.name = "ETF_DEMAND_PROXY"

                    print("ETF source: GLD shares outstanding")
                    print(etf_proxy.dropna().tail(5))
                else:
                    etf_proxy = None
            else:
                etf_proxy = None
            ...
            print("ETF shares branch failed:", e)
            etf_proxy = None

        # 2) Fallback: GLD 3M return proxy
        if etf_proxy is None or etf_proxy.dropna().empty:
            try:
                gld_s = pd.to_numeric(gld_ser, errors="coerce").dropna().copy()
                gld_s.index = pd.to_datetime(gld_s.index, errors="coerce")

                if getattr(gld_s.index, "tz", None) is not None:
                    gld_s.index = gld_s.index.tz_convert(None)

                gld_s = gld_s[~gld_s.index.isna()].sort_index()
                gld_m = gld_s.resample("ME").last()
                etf_proxy = 100.0 * (gld_m / gld_m.shift(3) - 1.0)
                etf_proxy.name = "ETF_DEMAND_PROXY"

                print("ETF source: GLD 3M return fallback")
                print(etf_proxy.dropna().tail(5))
            except Exception as e:
                print("ETF fallback branch failed:", e)
                etf_proxy = None

        # 3) Final assign
        if etf_proxy is not None and not etf_proxy.dropna().empty:
            etf_proxy = etf_proxy.rolling(2, min_periods=1).mean()

            print("ETF proxy pre-reindex tail:")
            print(etf_proxy.dropna().tail(5))
            print("ETF proxy tz:", getattr(etf_proxy.index, "tz", None))
            print("DF index tz:", getattr(df.index, "tz", None))

            df["ETF_DEMAND_PROXY"] = etf_proxy.reindex(df.index)
        else:
            df["ETF_DEMAND_PROXY"] = np.nan
            st.warning("GLD ETF proxy could not be constructed from shares outstanding or fallback proxy.")

    else:
        df["ETF_DEMAND_PROXY"] = np.nan
        st.warning(f"GLD ETF proxy load failed: {gld_err}")

    print("ETF latest values:")
    print(df["ETF_DEMAND_PROXY"].dropna().tail(5))

    # ----- Central bank demand proxy (FX reserves based) -----
    if "CB_DEMAND_PROXY" not in df.columns:
        fx_reserves = to_monthly(safe_fred("TRESEGUSM052N"), "avg")

        # Level change: slower structural accumulation
        cb_yoy = fx_reserves.diff(12).rolling(3, min_periods=1).mean()

        # Flow acceleration: catches renewed buying waves
        cb_accel = cb_yoy - cb_yoy.shift(6)

        # Blend: keep structure, add responsiveness
        cb_demand = 0.7 * cb_yoy + 0.3 * cb_accel
        cb_demand.name = "CB_DEMAND_PROXY"

        cb_demand = cb_demand.reindex(df.index).ffill(limit=3)
        df["CB_DEMAND_PROXY"] = cb_demand

        df["CB_DEMAND_PROXY"] = cb_demand

        # === GEO: VIX (geopolitics / stress proxy) ===
        # === GEO: VIX (robust version) ===
        try:
            import yfinance as yf

            vix = yf.download("^VIX", period="max", progress=False)["Close"]
            ...
            print("VIX loaded OK:", df["VIX"].notna().sum())

        except Exception as e:
            print("VIX load failed:", e)
            df["VIX"] = np.nan

        print("CB (FX reserves) latest values:")
        print(df["CB_DEMAND_PROXY"].dropna().tail(5))

    # =========================
    # GOLD MOMENTUM (CRITICAL)
    # =========================

    if "GOLD_USD" in df.columns:
        gold = df["GOLD_USD"].copy()

        # 12-month momentum (log-based for stability)
        gold_log = np.log(gold)
        gold_mom = gold_log - gold_log.shift(12)

        # Z-score normalization (rolling)
        z_window = 36
        z_gold_mom = (gold_mom - gold_mom.rolling(z_window).mean()) / gold_mom.rolling(z_window).std()

    else:
        df["Z_GOLD_MOM"] = np.nan


    # --- FAIR GOLD (monthly fair-value model)
    if ("GOLD_USD" in df.columns) and df["GOLD_USD"].dropna().shape[0] > 0:
        # Prefer TIPS real yield when available; otherwise use long-history proxy
        if "REAL_YIELD_TIPS10" in df.columns and df["REAL_YIELD_TIPS10"].dropna().shape[0] > 24:
            fair_real_yield = df["REAL_YIELD_TIPS10"].copy()
        else:
            fair_real_yield = df["REAL_YIELD_PROXY"].copy()

        # Safe stress input (HY OAS may occasionally be missing)
        if "HY_OAS" in df.columns and df["HY_OAS"].dropna().shape[0] > 24:
            # tress = level + shock component
            hy = df["HY_OAS"].copy()

            # Shock = 3M change (captures sudden stress)
            hy_shock = hy - hy.shift(3)

            # Combine level + shock (weighted)
            stress_input = hy + 0.7 * hy_shock
        else:
            stress_input = pd.Series(index=df.index, data=0.0)

        liq_input = (
            df["LIQ_12M_CHG"]
            if ("LIQ_12M_CHG" in df.columns and df["LIQ_12M_CHG"].dropna().shape[0] > 24)
            else pd.Series(index=df.index, data=0.0)
        )

        print("Columns before fair model:", df.columns.tolist())
        print("Z_GOLD_MOM exists:", "Z_GOLD_MOM" in df.columns)

        fair_tbl = compute_fair_gold_monthly(
            gold_m=df["GOLD_USD"],
            real_yield=fair_real_yield,
            usd_idx=df["USD_TWEX_SPLICE"],
            infl_proxy=df["INFL_EXP_PROXY"],
            stress_proxy=stress_input,
            liquidity_proxy=liq_input,
            etf_proxy=df["ETF_DEMAND_PROXY"],
            cb_proxy=df["CB_DEMAND_PROXY"],
            vix_series=df["VIX"],
            z_window=36,
            fit_window_months=72,
            min_fit_obs=24,
        )

        for c in fair_tbl.columns:
            df[c] = fair_tbl[c]

        # Daily aligned fair gold (step-like, monthly fair value forward-filled to daily index)
        if isinstance(gold_daily, pd.Series) and not gold_daily.dropna().empty:
            gd = gold_daily.dropna().copy()
            gd.index = pd.to_datetime(gd.index, errors="coerce")
            gd = gd[~pd.isna(gd.index)]

            fg_d = df["FAIR_GOLD"].reindex(gd.index, method="ffill")
            fg_gap_d = df["FAIR_GOLD_GAP_PCT"].reindex(gd.index, method="ffill")

            df.attrs["fair_gold_daily"] = fg_d
            df.attrs["fair_gold_gap_daily"] = fg_gap_d

        # Intraday aligned fair gold (step-like, monthly fair value forward-filled to intraday index later if available)
        df.attrs["fair_gold_monthly_raw"] = df["FAIR_GOLD"].dropna().copy()
        df.attrs["fair_gold_z_monthly_raw"] = df["FAIR_GOLD_Z"].dropna().copy()
...
def get_historical_asset_playbook(crisis_name: str, fit_score: float) -> str:
    """
    Historical analogue asset guidance.
    This is NOT a forecast. It summarizes what tended to work best in that type of regime.
    """
    ...
    if crisis_name == "1929 Great Depression":
        return (
            f"The current setup {strength} resembles a deflationary-deleveraging regime. "
            "Historically, the best defensive posture was to hold more cash, reduce equity risk, "
            "favor high-quality sovereign bonds early in the stress phase, and treat gold as a later-stage hedge "
            "once policy and monetary instability became more visible. Commodities and cyclical assets were usually weak."
        )

    elif crisis_name == "1974 Oil Shock":
        return (
            f"The current setup {strength} resembles an inflation / commodity shock regime. "
            "Historically, the better ideas were to favor hard assets, energy / oil-sensitive exposures, "
            "and gold, while being more cautious on long-duration bonds and broad equities. "
            "Cash helped tactically, but inflation protection mattered more than nominal safety."
        )

    elif crisis_name == "1980 Volcker Shock":
        return (
            f"The current setup {strength} resembles a high-inflation / aggressive-tightening regime. "
            "Historically, the better ideas were to prefer cash and short-duration USD assets, avoid long bonds, "
            "be cautious on gold after inflation peaks, and avoid weak-duration / rate-sensitive assets until real yields stabilized."
        )

    elif crisis_name == "2011 Euro Crisis":
        return (
            f"The current setup {strength} resembles a sovereign / policy stress regime. "
            "Historically, the better ideas were to prefer USD strength, high-quality government bonds, and selective gold exposure, "
            "while being cautious on peripheral-risk assets, banks, and cyclical equity beta."
        )

    elif crisis_name == "2020 Pandemic":
        return (
            f"The current setup {strength} resembles a liquidity-shock-then-stimulus regime. "
            "Historically, the better ideas were to hold cash early, then rotate toward gold and liquidity-supported risk assets "
            "once policy response became clear. Long bonds helped early; later, stimulus beneficiaries outperformed."
        )

    return (
        f"The current setup {strength} resembles the selected crisis only partially. "
        "Historically, the best approach in uncertain analogues was to stay diversified, hold some liquidity, "
        "and let the strongest current drivers (rates, USD, stress, inflation) determine whether to favor gold, bonds, cash, or cyclicals."
    )

def get_fit_color_label(score):
    if score >= 80:
        return "🔴 Strong Match"
    elif score >= 60:
...
def compute_crisis_fit_score(
    ...
    """
    Weighted similarity score (0-100) between today's macro profile
    and the selected crisis window.

    For each active indicator:
    - compare today's value to the crisis-window median
    - scale by crisis-window IQR (fallback to std)
    - convert distance to similarity via exp(-distance/scale)
    ...
    Returns:
        {
            ...
            "coverage_used": int,
            "coverage_total": int,
            "by_indicator": {key: similarity_0_1},
        }
    """
    start_dt = pd.to_datetime(win_start)
    end_dt = pd.to_datetime(win_end)
    ...
    for k in keys:
        ...
        dist = abs(float(cur) - med) / scale
        sim = float(np.exp(-dist))  # 1 = very similar, tends to 0 as distance rises
...
def build_current_contributions_bar_figure(
    ...
    """
    Horizontal signed bar chart of latest contributions.
    """
    items = []
    for k in core_keys:
    ...
    for v in vals:
        if v == 0 or pd.isna(v):
            ...
            plot_vals.append(0.01 if v > 0 else -0.01)
        else:
            plot_vals.append(v)

    fig, ax = plt.subplots(figsize=(7, max(3.2, 0.45 * len(names) + 1.5)))

    colors = ["#2ca02c" if v >= 0 else "#d62728" for v in vals]
    ax.barh(y, plot_vals, color=colors, alpha=0.85)
    ax.axvline(0, color="black", linewidth=0.8)

    ax.set_yticks(y)
    ax.set_yticklabels(names, fontsize=9)
    ax.invert_yaxis()
    ax.set_xlabel("Contribution to current crisis score")
    ax.set_title(title, fontsize=12)
    ax.grid(True, axis="x", alpha=0.3)

    # value labels
    pad = max(abs(min(vals)), abs(max(vals))) * 0.03 if vals else 0.02
    for yi, v in zip(y, vals):
        if v >= 0:
            ax.text(v + pad, yi, f"{v:+.2f}", va="center", fontsize=8)
        else:
            ax.text(v - pad, yi, f"{v:+.2f}", va="center", ha="right", fontsize=8)
...
- **Market Acceleration (today):** Evaluates whether market dynamics (momentum, positioning, technical signals) are accelerating or slowing gold price movements in the short term.
...
This section does **not** change the raw data itself. It mainly changes **the window through which you look at the data**.
    ...
    lookback_label = st.selectbox(
        ...
        index=lookback_label_options.index("240") if "60" in lookback_label_options else 0
    )
...
This is mainly useful for timing and monitoring, not for the slow macro regime itself.
        ...
        if mode == "Market Acceleration (fast)" and show_intraday_rsi:
            ...
            if isinstance(gold_intra, pd.Series) and not gold_intra.dropna().empty:
                ...
                # Fair Gold aligned to intraday timestamps (forward-filled monthly fair value)
                fair_month = res_base.attrs.get("fair_gold_monthly_raw", None)
                fair_gap_month = res_base.get("FAIR_GOLD_GAP_PCT") if "FAIR_GOLD_GAP_PCT" in res_base.columns else None

                if isinstance(fair_month, pd.Series) and not fair_month.dropna().empty:
                    fg_intra = fair_month.reindex(gold_intra.index, method="ffill")
                    res_base.attrs["fair_gold_intraday"] = fg_intra

                    if isinstance(fair_gap_month, pd.Series) and not fair_gap_month.dropna().empty:
                        fg_gap_intra = fair_gap_month.reindex(gold_intra.index, method="ffill")
                        res_base.attrs["fair_gold_gap_intraday"] = fg_gap_intra
...
def run_crisis(
    ...
    win_start: str,
    win_end: str,
) -> tuple[pd.DataFrame, list, dict, dict, dict, dict, dict]:
    ...
    if not thresholds:
        ...
        return res, core_keys, {}, dirs, weights, gold_stats, crisis_fit
    ...
    return res, core_keys, thresholds, dirs, weights, gold_stats, crisis_fit
...
if mode == "Structural Regime (today)":
    ...
    res, core_keys, thresholds, dirs, weights_used, gold_stats, crisis_fit = run_crisis(
        res_base,
        weights_crisis,
        dirs_crisis,
        win_start,
...
fair_gap_now = float(res_base["FAIR_GOLD_GAP_PCT"].dropna().iloc[-1]) if (
    "FAIR_GOLD_GAP_PCT" in res_base.columns and res_base["FAIR_GOLD_GAP_PCT"].dropna().shape[0] > 0
) else np.nan
...
**When the world looked like this crisis template before, what did gold usually do next?**  
...
**Crisis Fit** measures how similar today's macro profile is to the selected crisis template based on the active indicators. It is a similarity score, not a probability forecast.

**Color scale:**
- 🔴 **80–100%** → Strong match (very similar macro environment)
- 🟠 **60–80%** → High similarity
- 🟡 **40–60%** → Partial similarity (mixed signals)
- 🔵 **20–40%** → Weak similarity
- ⚪ **0–20%** → No meaningful resemblance

**How the overall Crisis Fit is calculated**

The app compares today's macro values with the historical values inside the selected crisis window.
For each active indicator:
1. It looks at the crisis-window distribution.
2. It finds the crisis median and spread (mainly the interquartile range, IQR).
3. It measures how far today's value is from that crisis median.
4. It converts that distance into a similarity score.

**Indicator similarity**  
  similarity = exp( - |today - crisis median| / crisis spread )

**Overall Crisis Fit %**  
  The app takes the weighted average of all active indicator similarities and multiplies by 100.
...
if mode == "Market Acceleration (fast)":
    ...
    if mode == "Crisis Similarity (template)":
        ...
        fit_pct = crisis_fit.get("fit_pct", np.nan)
        coverage_used = crisis_fit.get("coverage_used", 0)
        coverage_total = crisis_fit.get("coverage_total", 0)
        ...
        c6.metric("Core data through", str(res.index.max().date()))

        if not pd.isna(fit_pct):
            if fit_pct >= 80:
                fit_text = "strong resemblance to the selected historical crisis template."
            elif fit_pct >= 60:
                fit_text = "moderate resemblance to the selected historical crisis template."
            elif fit_pct >= 40:
                fit_text = "partial resemblance to the selected historical crisis template."
            elif fit_pct >= 20:
                fit_text = "weak resemblance to the selected historical crisis template."
            else:
                fit_text = "very different from the selected historical crisis template."
            ...
            asset_text = get_historical_asset_playbook(
                crisis_name=crisis,
                fit_score=fit_pct,
            )
...
In plain words: this is the section that helps you understand the **story over time**.
    ...
    with h4:
        st.markdown("**Current Indicator Influence (crisis score)**")

        fig_bar = build_current_contributions_bar_figure(
            latest_row=res_panel.iloc[-1],
            core_keys=core_keys,
            labels_map=labels,
            title="How much each indicator influences the current crisis score",
        )
...
def render_gold_action_panels(
    ...
    def render_indicator_group(group_title, cols_to_show):
        ...
        for row in chunked(shown_cols, 3):
            cards = st.columns(3)
            for i, col in enumerate(row):
                with cards[i]:
                    ...
                    if col in res.columns:
                        ...
                        use_crisis_compare = (
                            mode == "Crisis Similarity (template)"
                            and win_start is not None
                            and win_end is not None
                            and col in res_base.columns
                        )

                        if use_crisis_compare:
                            full_series = res_base[col].dropna().sort_index()

                            # current window
                            if lookback is None:
                                current_window = full_series.copy()
                            else:
                                current_window = full_series.tail(int(lookback))

                            # crisis window
                            crisis_window = full_series.loc[pd.to_datetime(win_start):pd.to_datetime(win_end)].dropna()
                            ...
                            fit_pct_ind = sim_meta.get("fit_pct", np.nan)
                            fit_label_ind = sim_meta.get("label", "No data")
                            cur_pctile = sim_meta.get("current_percentile", np.nan)
                            crisis_med = sim_meta.get("median", np.nan)

                            if not pd.isna(fit_pct_ind):
                                med_txt = "—" if pd.isna(crisis_med) else f"{crisis_med:.2f}"
                                pct_txt = "—" if pd.isna(cur_pctile) else f"{cur_pctile:.0f}th"
                                st.caption(
                                    f"Similarity to {crisis}: {fit_pct_ind:.0f}% ({fit_label_ind}) "
                                )
                            else:
                                st.caption(f"Similarity to {crisis}: unavailable ({fit_label_ind})")

                            higher_is_bullish = None
                            if is_scored:
                                if market_scored and not gold_scored and col in dirs_market:
                                    higher_is_bullish = dirs_market.get(col, None)
                                else:
                                    higher_is_bullish = dirs.get(col, None)

                            if current_window.dropna().shape[0] >= 3 and crisis_window.dropna().shape[0] >= 3:
                                fig = build_now_vs_crisis_vertical_figure(
                                    current_series=current_window,
                                    crisis_series=crisis_window,
                                    current_label="Now",
                                    crisis_label=f"{crisis}",
                                    title=labels.get(col, col),
                                    sim_meta=sim_meta,
                                    higher_is_bullish=higher_is_bullish,
                                )

                        # fallback: normal current-only sparkline
                        if fig is None:
                            s = res[col]
...
**Interpretation Guide for the GAP Chart**
| Gap        | Meaning                   |
| ---------- | ------------------------- |
| > +15%     | gold expensive vs macro   |
| +5 to +15% | late bull                 |
| -5 to +5%  | fair value                |
| -5 to -15% | undervalued               |
| < -15%     | deep macro undervaluation |
        ...
        fair_d_full = (
            res_base.attrs.get("fair_gold_daily", pd.Series(dtype=float)).dropna().sort_index()
            if isinstance(res_base.attrs.get("fair_gold_daily", None), pd.Series)
            else pd.Series(dtype=float)
        )

        fair_m_full = (
            res_base["FAIR_GOLD"].dropna().sort_index()
            if "FAIR_GOLD" in res_base.columns else pd.Series(dtype=float)
        )

        # ---- Align Fair Gold exactly with gold window ----
        if start_dt is None:
            fair_d_win = fair_d_full.reindex_like(gold_d_full, method="ffill")
            fair_m_win = fair_m_full
        else:
            fair_d_win = fair_d_full.reindex_like(gold_d_win, method="ffill")
            fair_m_win = fair_m_full.loc[fair_m_full.index >= start_dt] if not fair_m_full.empty else fair_m_full

        if not fair_d_win.empty and not fair_m_win.empty:
            fair_m_like = fair_m_win.copy()
            fair_m_like.loc[fair_d_win.index.max()] = float(fair_d_win.iloc[-1])
            fair_m_win = fair_m_like.sort_index()
        ...
        with mid:
            ...
            if isinstance(fair_d_win, pd.Series) and not fair_d_win.empty:
                ax.plot(
                    fair_d_win.index,
                    fair_d_win.values,
                    linewidth=1.3,
                    linestyle="--",
                    color="tab:purple",
                    label="Fair Gold (daily-aligned)"
                )

            if isinstance(fair_m_win, pd.Series) and not fair_m_win.empty:
                ax.plot(
                    fair_m_win.index,
                    fair_m_win.values,
                    linewidth=1.0,
        ...
        # ---------- FAIR GOLD DRIVER CONTRIBUTIONS ----------
        contrib_cols = [
            "CONTRIB_REAL_YIELD",
            "CONTRIB_USD",
            "CONTRIB_INFL",
        ...
        contrib_df = res_base[contrib_cols].copy() if all(c in res_base.columns for c in contrib_cols) else pd.DataFrame()

        if not contrib_df.empty:
            if start_dt is None:
                contrib_win = contrib_df.copy()
            else:
                contrib_win = contrib_df.loc[contrib_df.index >= start_dt].copy()

            contrib_win = contrib_win.dropna(how="all")

            if not contrib_win.empty:
                c_left, c_right = st.columns(2)

                with c_left:
                    fig, ax = plt.subplots(figsize=(6.0, 3.2))

                    plot_cols = [c for c in contrib_cols if c in contrib_win.columns]
                    ys = [contrib_win[c].fillna(0.0).values for c in plot_cols]
                    labels_ = [contrib_labels[c] for c in plot_cols]

                    ax.stackplot(contrib_win.index, ys, labels=labels_, alpha=0.85)
                    ...
                    # diag_cols = [
                    #     c for c in [
                    #         "BETA_ETF", "Z_ETF", "CONTRIB_ETF", "ETF_INPUT_LATEST",
                    #         "BETA_CB", "Z_CB", "CONTRIB_CB", "CB_INPUT_LATEST"
                    #     ] if c in res_base.columns
                    # ]
                    #
                    # if diag_cols:
                    #     latest_diag = res_base[diag_cols].dropna(how="all")
                    #     if not latest_diag.empty:
                    #         st.caption("Latest ETF / Central Bank diagnostics")
                    #         st.dataframe(
                    #             latest_diag.tail(1).T.rename(columns={latest_diag.tail(1).index[-1]: "Latest"}),
                    #             use_container_width=True
                    #         )

                with c_right:
                    latest_contrib = contrib_win.iloc[-1].fillna(0.0)

                    fig, ax = plt.subplots(figsize=(6.0, 3.2))

                    vals = latest_contrib.values
                    names = [contrib_labels[c] for c in latest_contrib.index]
        ...
        if show_intraday_rsi:
            ...
            if not (isinstance(rsi_intra, pd.Series) and not rsi_intra.dropna().empty) or not (
                ...
                with i_right:
                    ...
                    # ---- Fair Gold intraday alignment ----
                    fair_intra = res_base.attrs.get("fair_gold_intraday", None)

                    if isinstance(fair_intra, pd.Series) and not fair_intra.dropna().empty:
                        fair_intra = fair_intra.dropna().sort_index()
                        fair_intra_win = fair_intra.loc[
                            (fair_intra.index >= start_i) & (fair_intra.index <= end_i)
                            ]
                    else:
                        fair_intra_win = pd.Series(dtype=float)

                    fig, ax = plt.subplots(figsize=(6.0, 3.2))
                    ax.plot(gold_intra_win.index, gold_intra_win.values, linewidth=1.2, label=f"Gold ({intraday_interval})")

                    if not fair_intra_win.empty:
                        ax.plot(
                            fair_intra_win.index,
                            fair_intra_win.values,
                            linewidth=1.2,
                            linestyle="--",
                            color="tab:purple",
                            label="Fair Gold"
        ...
        with right:
            gap_full = (
                res_base["FAIR_GOLD_GAP_PCT"].dropna().sort_index()
                if "FAIR_GOLD_GAP_PCT" in res_base.columns else pd.Series(dtype=float)
            )
            if start_dt is None:
                gap_win = gap_full
            else:
                gap_win = gap_full.loc[gap_full.index >= start_dt] if not gap_full.empty else gap_full

            fig_gap, ax = plt.subplots(figsize=(6.0, 3.2))
            if not gap_win.empty:
                ...
                # Shade zones only if there is data in the selected window
                ymax = float(gap_win.max()) if gap_win.shape[0] else 10.0
                ymin = float(gap_win.min()) if gap_win.shape[0] else -10.0
            ...
            ax.xaxis.set_major_locator(mdates.AutoDateLocator())
            ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))
            fig_gap.autofmt_xdate(rotation=30, ha="right")
...
It creates many possible future price paths based on similar past market states. The goal is to show a reasonable range of outcomes, including a typical path, a weaker path, and a stronger path.
        ...
        It helps you compare which assets look more favorable, less favorable, or more uncertain under their current tactical setup.
                            ...
                            st.dataframe(mc_summary_df,width="stretch",hide_index=True)
    ...
    - **Asset** = the raw market move
        ...
        It helps you inspect where the forecast worked well, where it missed, and whether errors happened in specific market states.
                    """)
                st.dataframe(bt_df[mc_cols], width="stretch", hide_index=True)
    ...
    So a BUY signal does not always mean the trade was executed immediately.
            ...
            st.dataframe(replay_show, width="stretch", hide_index=True)
